/* File  Date.c
Realization of Date structure functions(Task 26)
...
void input_Date(TIME* D) {
	scanf_s("%04d/%02d/%02d %02d:%02d:%02d", &D->tm_year - 1900, &D->tm_mon - 1, &D->tm_mday, &D->tm_hour, &D->tm_min, &D->tm_sec);
	D->tm_year -= 1900;
	D->tm_mon--;
}
...
void inputTextFile_Date(FILE* file, TIME* D) {
	fscanf_s(file, "%04d/%02d/%02d %02d:%02d:%02d", &D->tm_year, &D->tm_mon, &D->tm_mday, &D->tm_hour, &D->tm_min, &D->tm_sec);
	D->tm_year -= 1900;
	D->tm_mon--;
}
